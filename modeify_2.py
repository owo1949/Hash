...
import trimesh
import wandb
from hit.utils.smpl_utils import get_template_verts, mpimesh2glb
from hit.utils.tensors import cond_create, eik_loss
...
def compute_occupancy_loss(hit_pl, batch, pred_occ):
	...
	focal_criterion = FocalLoss(alpha=manual_weights, gamma, reduction='mean')
...
def Bdense_loss(x_s, x_c, B_xc, beta):
    fwd_disp = torch.matmul(B_xc.transpose(-2, -1), beta.unsqueeze(1).repeat(1, x_c.shape[1], 1).unsqueeze(-1)).squeeze(
        -1)  # B, N, 3
...
def compute_linear_beta_loss(hit_pl, batch, cond, smpl_output, pts_c_inside, on_surface=False):
    ...
    linear_beta_loss = Bdense_loss(x_s, x_c, B_xc,
                                   batch['betas'])  # Forces the displacement to be a linear function of beta
...
def pose_bs_loss(hit_pl, batch, batch_idx, cond):
    ...
    smpl_posed = hit_pl.smpl.forward(zero_beta, body_pose=batch['body_pose'], global_orient=batch['global_orient'],
                                     transl=batch['transl'])
...
def compute_outside_can_loss(hit_pl, smpl_output, batch, batch_idx):
    ...
    gt_outside_cc = torch.ones(nb_pts_outside).to(
        uniform_can_pts_pred.device)  # The class of all those points should be zero
    # import ipdb; ipdb.set_trace()
...
def compute_hands_loss(hit_pl, batch):
    ...
    loss_hands = F.cross_entropy(pred_hands.reshape(-1, len(hit_pl.train_cfg.mri_labels)),
...
def compute_lbs_regularization_loss(hit_pl, batch, batch_idx, cond, step='train'):
    ...
    gt_weights = hit_pl.smpl.lbs_weights.unsqueeze(0).expand(batch['betas'].shape[0], -1, -1).to(
        smpl_template_verts.device)
    # import ipdb; ipdb.set_trace()
...
def compute_surface_loss(hit_pl, batch, batch_idx, cond):
    ...
    pred_occ_NO = torch.softmax(pred_occ, dim=-1)[
        ..., 0]  # softmax over the tissues, then take the first channel value (NO)
...
def compute_comp0_loss(hit_pl, batch, batch_idx, smpl_output, cond, pts_c, occ_pred):
    ...
    bone_comp_loss = F.mse_loss(d_pred[bone_mask], torch.zeros_like(
        d_pred[bone_mask]))  # Convert to cm with *100 for better numerical stability
    lt_comp_loss = F.mse_loss(d_pred[lt_mask], torch.zeros_like(
        d_pred[lt_mask]))  # Convert to cm with *100 for better numerical stability
    ...
    if hit_pl.train_cfg['comp0_out']:
        ...
        d_pred = hit_pl.hit.deformer.compressor(xf_s, cond)
        # import ipdb; ipdb.set_trace()
        comp0_loss_out = F.mse_loss(d_pred[mri_out_pts] * 100, torch.zeros_like(
            d_pred[mri_out_pts]))  # Convert to cm with *100 for better numerical stability
...
def compute_mri_loss(hit_pl, batch, batch_idx, smpl_output, cond, pts_c_inside):
    ...
    _, image = sl.plot_slice_value(mri_value, to_plot=False,
                                   mri_values=True)  # This is the SMPL frontal cut image of the mri value
